	...
	//		Modified:2008/11/13
	//		Author:??
	//		TaskName:???NPC????????????318?NPC
	//		TaskID:HD-jinzita-NPC-WHtoMS.s
	//
	//****************************************
	//??????
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $boss_wh_kill == 0
			if $boss_ms_kill == 0
				BC( "dialogbox", "player", -1, "Shadow of the Queen's power hasn't vanished, it can be transferred after the death of Prince Arbyss or Shadow of the Queen!" )
				return
			endif
		endif
		//if $boss_js_kill == 0
		...
		//if $count < 1
		//	BC( "dialogbox", "player", -1, "GUID:04997000001" )
		//	return
		//endif
