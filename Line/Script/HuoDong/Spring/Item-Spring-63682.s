...
//---------------------------------
		if $time == $nowtime
				if $round <= 5
						$info = SubPlayerInfo( -1 , "item" , 63682 , 1 )
						if  $info != 0
								return
						endif
						if $lv < 45
				...
				BC( "screen", "player", -1, "You have tasted a Lucky Dumpling." )
			else
				BC( "screen", "player", -1, "You have tasted 5 Lucky Dumpling today. Don't be greedy." )
				return
			endif
		else
				$info = SubPlayerInfo ( -1 , "item" , 63682 , 1 )
