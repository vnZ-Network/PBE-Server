	...
	//		Modified:2008/11/13
	//		Author:??
	//		TaskName:???NPC????1736?1752?NPC
	//		TaskID:HD-jinzita-NPC-MStoBOSS.s
	//
	//****************************************
	//??????
	function OnRequest(){
		
		$wh_key_number = GetServerVar (42)
	...
	function OnOption0(){
		...
		if $count < 1
			BC( "dialogbox", "player", -1, "You do not have a Key to Maze of Mystery." )
			return
		endif
		$revalue = SubPlayerInfo(-1 , "item" , 63182 , 1)
  		if $revalue != 0
			return
		endif
		...
		SetServerVar (42 , $wh_key_number)
		//????????,?????
		$a = 100 - $wh_key_number
		#player_name = GetPlayerInfo( -1, "name" )
		if $a > 0
			BC( "screen", "map", 125, "The hero ",#player_name," donated a key to the Maze of Mystery. " ,$a , " more keys are needed.")
		else
			BC( "screen", "map", 125, "The hero ",#player_name," has donated the last key to the Maze of Mystery. 100 keys have been donated and the path to the boss is open!")
		endif
