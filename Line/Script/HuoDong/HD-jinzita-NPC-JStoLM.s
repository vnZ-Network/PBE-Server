	...
	//		Modified:2008/11/13
	//		Author:??
	//		TaskName:???NPC???????????320?NPC
	//		TaskID:HD-jinzita-NPC-JStoLM.s
	//
	//****************************************
	//??????
	function OnRequest(){
	...
	//	if $count < 1
	//		BC( "dialogbox", "player", -1, "GUID:04967000000" )
	//		return
	//	endif
		...
		if $boss_js_kill == 0
			if $boss_lm_kill == 0
				BC( "dialogbox", "player", -1, "Molmes' power hasn't vanished, can be transferred after the death of Molmes or Arch Priest Britacesk!" )
				return
			endif
		endif
