	...
	//		Modified:2010/6/17
	//		Author:??
	//		TaskName:???NPC??????????1061?NPC
	//		TaskID:HD-jinzita-NPC-WHtoJS.s
	//
	//****************************************
	//??????
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $boss_wh_kill == 0
			if $boss_js_kill == 0
				BC( "dialogbox", "player", -1, "Shadow of the Queen's power hasn't vanished, it can be transferred after the death of Molmes or Shadow of the Queen!" )
				return
			endif
		endif
		//if $boss_js_kill == 0
		...
		//if $count < 1
		//	BC( "dialogbox", "player", -1, "GUID:04996000001" )
		//	return
		//endif
