	...
	//		TaskID:HD-jinzita-NPC-MStoLM.s
	//
	//****************************************
	//??????
	function OnRequest(){
	...
	function OnOption0(){
		...
		if $boss_ms_kill == 0
			if $boss_lm_kill == 0
				BC( "dialogbox", "player", -1, "Prince Arbyss' power hasn't vanished, it can be transferred after the death of Prince Arbyss or Arch Priest Britacesk!" )
				return
			endif
		endif
		//if $boss_js_kill == 0
		...
		//if $count < 1
		//	BC( "dialogbox", "player", -1, "GUID:04992000001" )
		//	return
		//endif
