...
//───────────────────────────────────────
    ...
    //"Check Thông Số"
    //───────────────────────────────────
     #instanceEN = "Dragon Emeperor's (Easy)"
     #instanceVN = "Mộ Long Đế (Dễ)"
          $EctVar = 7
          $Dif = 0
...
//───────────────────────────────────────
    ...
    //"Check Thông Số"
    //───────────────────────────────────
     #instanceEN = "Dragon Emeperor's (Hard)"
     #instanceVN = "Mộ Long Đế (Khó)"
          $EctVar = 7
          $Dif = 1
...
//───────────────────────────────────────
    ...
    //"Check Thông Số"
    //───────────────────────────────────
     #instanceEN = "Dragon Emeperor's (Nightmare)"
     #instanceVN = "Mộ Long Đế (Địa Ngục)"
          $EctVar = 7
          $Dif = 2
...
//───────────────────────────────────
     function OnOption3(){
          ...
          $is_succeed = FlyToEctype( -1, $mapID, $posX, $posY )
               if $lang == 1
                    BC( "messagebox", "player", -1, "{#FF30FFEE=Trở lại Phụ Bản thành công#}" )
               endif
               if $lang == 2
                    BC( "messagebox", "player", -1, "{#FF30FFEE=Return to Dungeon#}" )
               endif
          if $is_succeed > 0
          $Player_UID = GetPlayerVar( -1, $v_return )
               if $Player_UID == $ectypeUID
	...
	function OnOption4(){
		...
		if $DUNGEON > 0 
			...
			if $Complete != 0
				...
				if $lang == 2
					BC( "messagebox", "player", -1, "{#FFBF2271=Can't Reset Dungeon\nYour team still inside#}" )
					return
				endif
			endif
			...
			if $revalue == 0
				if $lang == 1
					BC( "messagebox", "player", -1, "{#FF30FFEE=Dungeon đã được làm mới#}" )
				endif
				if $lang == 2
					BC( "messagebox", "player", -1, "{#FF30FFEE=Dungeon has been Reset#}" )
				endif	
			else
				if $lang == 1
				...
				if $lang == 2
					BC( "chat", "player", -1, "[color=FFF23805]Failed to reset Dungeon" )
				endif
			endif
			return
		endif	
