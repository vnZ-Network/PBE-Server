	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1893.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 8
			...
			$Status = IsExistStatus( -1 , 10690 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Scorpio protection. You cannot acquire Scorpio reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status8 == 0
			BC( "dialogbox", "player", -1, "You already have the Scorpio protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
