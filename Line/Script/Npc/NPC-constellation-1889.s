	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1889.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 4
			...
			$Status = IsExistStatus( -1 , 10686 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Cancer protection. You cannot acquire Cancer reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status4 == 0
			BC( "dialogbox", "player", -1, "You already have Cancer protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
