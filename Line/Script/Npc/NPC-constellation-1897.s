	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1897.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 12
			...
			$Status = IsExistStatus( -1 , 10694 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Pisces protection. You cannot acquire Pisces reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status12 == 0
			BC( "dialogbox", "player", -1, "You already have the Pisces protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
