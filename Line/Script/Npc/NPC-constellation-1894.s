	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1894.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 9
			...
			$Status = IsExistStatus( -1 , 10691 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Sagittarius protection. You cannot acquire Sagittarius reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status9 == 0
			BC( "dialogbox", "player", -1, "You already have the Sagittarius protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
