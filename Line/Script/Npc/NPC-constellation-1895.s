	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1895.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 10
			...
			$Status = IsExistStatus( -1 , 10692 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Capricorn protection. You cannot acquire Capricorn reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status10 == 0
			BC( "dialogbox", "player", -1, "You already have the Capricorn protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
