	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1896.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 11
			...
			$Status = IsExistStatus( -1 , 10693 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Aquarius protection. You cannot acquire Aquarius reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status11 == 0
			BC( "dialogbox", "player", -1, "You already have the Aquarius protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
