	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1890.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 5
			...
			$Status = IsExistStatus( -1 , 10687 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Leo protection. You cannot acquire Leo reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status5 == 0
			BC( "dialogbox", "player", -1, "You already have the Leo protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
