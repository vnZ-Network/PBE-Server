	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1886.s
	//		TaskID: ?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 1
			...
			$Status = IsExistStatus( -1 , 10683 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Aries protection. You cannot acquire Aries reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status1 == 0
			BC( "dialogbox", "player", -1, "You already have the Aries protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
