...
function OnOption0(){
	...
	if $Guildid <= 0
		BC( "dialogbox", "player", -1, "You must be in a Guild to upgrade the Guild Meeting Hall." )
		return
	endif
	...
	if $Guildrank != 0
		BC( "dialogbox", "player", -1, "Only the Guild leader can upgrade the Guild Meeting Hall." )
		return
	endif
	
	//????????????
	$BuildingLevel = GetGuildBuildingLevel( $Guildid , 1 )
	
	if $BuildingLevel >= 5
		BC( "dialogbox", "player", -1, "The Guild Meeting Hall has already been upgraded to its maximum level." )
		return
	endif
	...
	if $BuildingLevel == $GuildLevel
		BC( "dialogbox", "player", -1, "The Guild Meeting Hall level cannot be higher than the Guild level." )
		return
	endif	
	
	$result = GuildBuildingLevelUp( $Guildid , 1 )
