...
function OnOption0(){
	...
	if $Guildid <= 0
		BC( "dialogbox", "player", -1, "You must be in a Guild to upgrade the Guild Arsenal." )
		return
	endif
	...
	if $Guildrank != 0
		BC( "dialogbox", "player", -1, "Only the Guild leader can upgrade the Guild Arsenal." )
		return
	endif
	
	//????????????
	$BuildingLevel = GetGuildBuildingLevel( $Guildid , 4 )
	
	if $BuildingLevel >= 5
		BC( "dialogbox", "player", -1, "The Guild Arsenal has already been upgraded to its maximum level." )
		return
	endif
	...
	if $BuildingLevel == $GuildLevel
		BC( "dialogbox", "player", -1, "The Guild Arsenal level cannot be higher than the Guild level." )
		return
	endif	
	
	$result = GuildBuildingLevelUp( $Guildid , 4 )
