...
function OnOption0(){
	...
	if $Guildid <= 0
		BC( "dialogbox", "player", -1, "You must be in a Guild to upgrade the Guild Bounty Tavern." )
		return
	endif
	...
	if $Guildrank != 0
		BC( "dialogbox", "player", -1, "Only the Guild leader can upgrade the Guild Bounty Tavern." )
		return
	endif
	
	//????????????
	$BuildingLevel = GetGuildBuildingLevel( $Guildid , 2 )
	
	if $BuildingLevel >= 5
		BC( "dialogbox", "player", -1, "The Guild Bounty Tavern has already been upgraded to its maximum level." )
		return
	endif
	...
	if $BuildingLevel == $GuildLevel
		BC( "dialogbox", "player", -1, "The Guild Bounty Tavern level cannot be higher than the Guild level." )
		return
	endif	
	
	$result = GuildBuildingLevelUp( $Guildid , 2 )
...
function OnOption2(){
	...
	if $LevelingBuilding == 2
		BC( "dialogbox", "player", -1, "The Guild Bounty Tavern is under construction.\nCurrent progress is " , $BuildingExp , "/" , $BuildingMaxExp,"\n{#ffc72b2b=Gather Construction Materials in the Guild Territory and bring them to the building under construction to increase the construction progress.#}" )
	endif	
