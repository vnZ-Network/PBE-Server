	...
	function OnOption0(){
		...
		$item = GetPlayerInfo( -1, "item", $ItemID1 )
		if $item > 0
			BC( "dialogbox", "player", -1, "You already have Incantation of Seal(Red)." )
			return
		endif
		...
		if $Item0 >= $ItemNum2
			...
			BC( "dialogbox", "player", -1, "You need {#ffff002a=",$ItemNum2," Raw Jade#} to duplicate the incantation of seal from the Red Crystal. {#ffff002a=(Use Incantation of Seal to locate the Red Crystaloid within the Red Crystal. Monsters below LV45 drop Raw Jade.)#}" )
			return
		endif
	...
	function OnOption1(){
		...
		if $Item0 >= $ItemNum3
			...
			BC( "dialogbox", "player", -1, "The seal on the Red Crystal has been removed!" )
			SetPlayerVar( -1, $var1, 1 )
		else
			BC( "dialogbox", "player", -1, "You need a {#ffff002a=Red Crystaloid#} to remove the seal on the Red Seal Crystal. {#ffff002a=Use Incantation of Seal (Red) to locate the Red Crystaloid within the Red Crystal.#}" )
			return
		endif
