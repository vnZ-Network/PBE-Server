	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1887.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 2
			...
			$Status = IsExistStatus( -1 , 10684 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Taurus protection. You cannot acquire Taurus reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status2 == 0
			BC( "dialogbox", "player", -1, "You already have the Taurus protection. Do you want even more rewards?" )
			return
		endif
		if $Status1 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
