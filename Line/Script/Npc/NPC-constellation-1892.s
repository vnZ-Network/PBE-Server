	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1892.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $a == 7
			...
			$Status = IsExistStatus( -1 , 10689 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Libra protection. You cannot acquire Libra reward." )
				return
			endif
	...
	function OnOption1(){
		...
		if $Status7 == 0
			BC( "dialogbox", "player", -1, "You already have the Libra protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
