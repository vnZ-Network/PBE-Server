	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4013 , $num )
		
			//???????????
			$servernum = GetServerVar( 229 )
			$servernum = $servernum + 1
			SetServerVar( 229 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4046 , $num )
		
			//???????????
			$servernum = GetServerVar( 262 )
			$servernum = $servernum + 1
			SetServerVar( 262 , $servernum )
