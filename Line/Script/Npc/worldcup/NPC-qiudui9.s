	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4003 , $num )
		
			//????????????
			$servernum = GetServerVar( 219 )
			$servernum = $servernum + 1
			SetServerVar( 219 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4036 , $num )
		
			//????????????
			$servernum = GetServerVar( 252 )
			$servernum = $servernum + 1
			SetServerVar( 252 , $servernum )
