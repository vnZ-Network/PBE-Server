	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4001 , $num )
		
			//???????????
			$servernum = GetServerVar( 217 )
			$servernum = $servernum + 1
			SetServerVar( 217 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4034 , $num )
		
			//???????????
			$servernum = GetServerVar( 250 )
			$servernum = $servernum + 1
			SetServerVar( 250 , $servernum )
