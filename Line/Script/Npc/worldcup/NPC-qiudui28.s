	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4022 , $num )
		
			//????????????
			$servernum = GetServerVar( 238 )
			$servernum = $servernum + 1
			SetServerVar( 238 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4055 , $num )
		
			//????????????
			$servernum = GetServerVar( 271 )
			$servernum = $servernum + 1
			SetServerVar( 271 , $servernum )
