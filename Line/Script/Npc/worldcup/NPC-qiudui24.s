	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4018 , $num )
		
			//?????????????
			$servernum = GetServerVar( 234 )
			$servernum = $servernum + 1
			SetServerVar( 234 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4051 , $num )
		
			//?????????????
			$servernum = GetServerVar( 267 )
			$servernum = $servernum + 1
			SetServerVar( 267 , $servernum )
