	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4017 , $num )
		
			//????????????
			$servernum = GetServerVar( 233 )
			$servernum = $servernum + 1
			SetServerVar( 233 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4050 , $num )
		
			//????????????
			$servernum = GetServerVar( 266 )
			$servernum = $servernum + 1
			SetServerVar( 266 , $servernum )
