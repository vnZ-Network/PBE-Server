...
function OnRequest(){
	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 3996 , $num )
		
			//????????????
			$servernum = GetServerVar( 212 )
			$servernum = $servernum + 1
			SetServerVar( 212 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4029 , $num )
		
			//????????????
			$servernum = GetServerVar( 245 )
			$servernum = $servernum + 1
			SetServerVar( 245 , $servernum )
