	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 3998 , $num )
		
			//???????????
			$servernum = GetServerVar( 214 )
			$servernum = $servernum + 1
			SetServerVar( 214 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4031 , $num )
		
			//???????????
			$servernum = GetServerVar( 247 )
			$servernum = $servernum + 1
			SetServerVar( 247 , $servernum )
