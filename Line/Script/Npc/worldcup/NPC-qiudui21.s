	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4015 , $num )
		
			//????????????
			$servernum = GetServerVar( 231 )
			$servernum = $servernum + 1
			SetServerVar( 231 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4048 , $num )
		
			//????????????
			$servernum = GetServerVar( 264 )
			$servernum = $servernum + 1
			SetServerVar( 264 , $servernum )
