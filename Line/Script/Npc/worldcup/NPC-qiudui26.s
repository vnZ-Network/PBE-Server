	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4020 , $num )
		
			//???????????
			$servernum = GetServerVar( 236 )
			$servernum = $servernum + 1
			SetServerVar( 236 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4053 , $num )
		
			//???????????
			$servernum = GetServerVar( 269 )
			$servernum = $servernum + 1
			SetServerVar( 269 , $servernum )
