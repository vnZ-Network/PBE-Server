	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4026 , $num )
		
			//???????????
			$servernum = GetServerVar( 242 )
			$servernum = $servernum + 1
			SetServerVar( 242 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4059 , $num )
		
			//???????????
			$servernum = GetServerVar( 275 )
			$servernum = $servernum + 1
			SetServerVar( 275 , $servernum )
