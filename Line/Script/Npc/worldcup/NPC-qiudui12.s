	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4006 , $num )
		
			//??????????????
			$servernum = GetServerVar( 222 )
			$servernum = $servernum + 1
			SetServerVar( 222 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4039 , $num )
		
			//??????????????
			$servernum = GetServerVar( 255 )
			$servernum = $servernum + 1
			SetServerVar( 255 , $servernum )
