	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4008 , $num )
		
			//?????????????
			$servernum = GetServerVar( 224 )
			$servernum = $servernum + 1
			SetServerVar( 224 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4041 , $num )
		
			//?????????????
			$servernum = GetServerVar( 257 )
			$servernum = $servernum + 1
			SetServerVar( 257 , $servernum )
