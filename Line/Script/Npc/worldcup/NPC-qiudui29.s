	...
	function OnOption0(){
		...
		$num = GetPlayerVar( -1 , 4023 )
		$no1num = GetPlayerVar( -1 , 4056 )
	...
	function OnOption1(){
		...
		//????????4???
		$need = GetPlayerVar( -1 , 3830 )
		//???????4????
		$all = GetPlayerVar( -1 , 3831 )
		
		if $all > 0
			if $need == 0
				BC( "dialogbox" , "player" , -1 , "Sorry, you have no receivable Fund" )
				return
			endif
		endif
		...
		$myday = GetPlayerVar( -1 , 4027 )
		if $Now_Yday == $myday
			BC( "dialogbox" , "player" , -1 , "Sorry, you have received Fund today." )
			return
		endif
		
		
		//???????????1?xxxx+1,4?4???,?????
		$first = GetPlayerVar( -1 , 3279 )
		if $first == 0
		...
		//??A?4????4?
		$allteam1 = GetServerVar( 213 )
		$allteam2 = GetServerVar( 223 )
		$allteam3 = GetServerVar( 227 )
		$allteam4 = GetServerVar( 239 )
		
		
		
		//??? - ???????
		$allmoney = $allmoney - $allteam1
		$allmoney = $allmoney - $allteam2
		$allmoney = $allmoney - $allteam3
		$allmoney = $allmoney - $allteam4
		
		//??????****
		$myteam = GetPlayerVar( -1 , 4023 )
		...
		$allmoney = $allmoney + $myteam
			...
			//????????????
			SetPlayerVar( -1 , 3830 , $allmoney )
			SetPlayerVar( -1 , 3831 , $allmoney )
			
			//??????
			SetPlayerVar( -1 , 3279 , 11 )
		...
		//????800
			if $need > 800
				...
				//????800,?800w
				AddPlayerInfo( -1 , "money" , 8000000 )
				$need = $need - 800
				//???????
				SetPlayerVar( -1 , 3830 , $need )
				
				BC( "dialogbox" , "player" , -1 , "N/A????????,?????" , $all , "N/A0000???,?????????8000000???,???" , $need , "N/A0000???????!" )
				SetPlayerVar( -1 , 4027 , $Now_Yday )
				
			else
				$need = $need * 10000
				AddPlayerInfo( -1 , "money" , $need )
				SetPlayerVar( -1 , 3830 , 0 )
				BC( "dialogbox" , "player" , -1 , "N/A??????????,???" , $need ,"N/A??!" )
				SetPlayerVar( -1 , 4027 , $Now_Yday )
	...
	function OnOption2(){
		...
		$need = GetPlayerVar( -1 , 3823 )
		$all = GetPlayerVar( -1 , 3825 )
		...
		//???????????1?
		$first = GetPlayerVar( -1 , 3280 )
		if $first == 0
		
		//??????
		$tol1 = GetServerVar( 244 )
		$tol2 = GetServerVar( 245 )
		$tol3 = GetServerVar( 246 )
		$tol4 = GetServerVar( 247 )
		$tol5 = GetServerVar( 248 )
		$tol6 = GetServerVar( 249 )
		$tol7 = GetServerVar( 250 )
		$tol8 = GetServerVar( 251 )
		$tol9 = GetServerVar( 252 )
		$tol10 = GetServerVar( 253 )
		$tol11 = GetServerVar( 254 )
		$tol12 = GetServerVar( 255 )
		$tol13 = GetServerVar( 256 )
		$tol14 = GetServerVar( 257 )
		$tol15 = GetServerVar( 258 )
		$tol16 = GetServerVar( 259 )
		$tol17 = GetServerVar( 260 )
		$tol18 = GetServerVar( 261 )
		$tol19 = GetServerVar( 262 )
		$tol20 = GetServerVar( 263 )
		$tol21 = GetServerVar( 264 )
		$tol22 = GetServerVar( 265 )
		$tol23 = GetServerVar( 266 )
		$tol24 = GetServerVar( 267 )
		$tol25 = GetServerVar( 268 )
		$tol26 = GetServerVar( 269 )
		$tol27 = GetServerVar( 270 )
		$tol28 = GetServerVar( 271 )
		$tol29 = GetServerVar( 272 )
		$tol30 = GetServerVar( 273 )
		$tol31 = GetServerVar( 274 )
		$tol32 = GetServerVar( 275 )
		...
		//??A?4????4?
		$allteam1 = GetServerVar( 272 )
		
		//??????		
		$myteam = GetPlayerVar( -1 , 4056 )
		...
		$allmoney = $allmoney + $myteam
			...
			//????????????
			SetPlayerVar( -1 , 3823 , $allmoney )
			SetPlayerVar( -1 , 3825 , $allmoney )
			
			//??????
			SetPlayerVar( -1 , 3280 , 11 )
		...
		//????800
			if $need > 800
				...
				$need = $need - 800
				//???????
				SetPlayerVar( -1 , 3823 , $need )
				
				BC( "dialogbox" , "player" , -1 , "N/A????????,?????" , $all , "N/A0000???,?????????8000000???,???" , $need , "N/A0000???????!" )
				SetPlayerVar( -1 , 4060 , $Now_Yday )
				
			else
				$need = $need * 10000
				AddPlayerInfo( -1 , "money" , $need )
				SetPlayerVar( -1 , 3823 , 0 )
				BC( "dialogbox" , "player" , -1 , "N/A??????????,???" , $need ,"N/A??!" )
				SetPlayerVar( -1 , 4060 , $Now_Yday )
