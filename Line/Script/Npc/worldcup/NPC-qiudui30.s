	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4024 , $num )
		
			//???????????
			$servernum = GetServerVar( 240 )
			$servernum = $servernum + 1
			SetServerVar( 240 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4057 , $num )
		
			//???????????
			$servernum = GetServerVar( 273 )
			$servernum = $servernum + 1
			SetServerVar( 273 , $servernum )
