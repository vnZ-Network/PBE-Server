	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4004 , $num )
		
			//???????????
			$servernum = GetServerVar( 220 )
			$servernum = $servernum + 1
			SetServerVar( 220 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4037 , $num )
		
			//???????????
			$servernum = GetServerVar( 253 )
			$servernum = $servernum + 1
			SetServerVar( 253 , $servernum )
