	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4021 , $num )
		
			//?????????????
			$servernum = GetServerVar( 237 )
			$servernum = $servernum + 1
			SetServerVar( 237 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4054 , $num )
		
			//?????????????
			$servernum = GetServerVar( 270 )
			$servernum = $servernum + 1
			SetServerVar( 270 , $servernum )
