	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4014 , $num )
		
			//????????????
			$servernum = GetServerVar( 230 )
			$servernum = $servernum + 1
			SetServerVar( 230 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4047 , $num )
		
			//????????????
			$servernum = GetServerVar( 263 )
			$servernum = $servernum + 1
			SetServerVar( 263 , $servernum )
