	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4002 , $num )
		
			//???????????
			$servernum = GetServerVar( 218 )
			$servernum = $servernum + 1
			SetServerVar( 218 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4035 , $num )
		
			//???????????
			$servernum = GetServerVar( 251 )
			$servernum = $servernum + 1
			SetServerVar( 251 , $servernum )
