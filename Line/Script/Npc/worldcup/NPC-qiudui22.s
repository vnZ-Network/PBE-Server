	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4016 , $num )
		
			//????????????
			$servernum = GetServerVar( 232 )
			$servernum = $servernum + 1
			SetServerVar( 232 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4049 , $num )
		
			//????????????
			$servernum = GetServerVar( 265 )
			$servernum = $servernum + 1
			SetServerVar( 265 , $servernum )
