	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4005 , $num )
		
			//??????????????
			$servernum = GetServerVar( 221 )
			$servernum = $servernum + 1
			SetServerVar( 221 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//??????????????
			$servernum = GetServerVar( 254 )
			$servernum = $servernum + 1
			SetServerVar( 254 , $servernum )
