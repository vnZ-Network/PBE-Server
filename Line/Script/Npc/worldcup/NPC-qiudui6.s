	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4000 , $num )
		
			//?????????????
			$servernum = GetServerVar( 216 )
			$servernum = $servernum + 1
			SetServerVar( 216 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4033 , $num )
		
			//?????????????
			$servernum = GetServerVar( 249 )
			$servernum = $servernum + 1
			SetServerVar( 249 , $servernum )
