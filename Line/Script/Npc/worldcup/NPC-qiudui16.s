	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4010 , $num )
		
			//???????????
			$servernum = GetServerVar( 226 )
			$servernum = $servernum + 1
			SetServerVar( 226 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4043 , $num )
		
			//???????????
			$servernum = GetServerVar( 259 )
			$servernum = $servernum + 1
			SetServerVar( 259 , $servernum )
