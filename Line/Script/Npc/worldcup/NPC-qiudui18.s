	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4012 , $num )
		
			//???????????
			$servernum = GetServerVar( 228 )
			$servernum = $servernum + 1
			SetServerVar( 228 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4045 , $num )
		
			//???????????
			$servernum = GetServerVar( 261 )
			$servernum = $servernum + 1
			SetServerVar( 261 , $servernum )
