	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4009 , $num )
		
			//?????????????
			$servernum = GetServerVar( 225 )
			$servernum = $servernum + 1
			SetServerVar( 225 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4042 , $num )
		
			//?????????????
			$servernum = GetServerVar( 258 )
			$servernum = $servernum + 1
			SetServerVar( 258 , $servernum )
