	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 3999 , $num )
		
			//????????????
			$servernum = GetServerVar( 215 )
			$servernum = $servernum + 1
			SetServerVar( 215 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4032 , $num )
		
			//????????????
			$servernum = GetServerVar( 248 )
			$servernum = $servernum + 1
			SetServerVar( 248 , $servernum )
