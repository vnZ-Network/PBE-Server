	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4019 , $num )
		
			//???????????
			$servernum = GetServerVar( 235 )
			$servernum = $servernum + 1
			SetServerVar( 235 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4052 , $num )
		
			//???????????
			$servernum = GetServerVar( 268 )
			$servernum = $servernum + 1
			SetServerVar( 268 , $servernum )
