	...
	function OnOption0(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4025 , $num )
		
			//?????????????
			$servernum = GetServerVar( 241 )
			$servernum = $servernum + 1
			SetServerVar( 241 , $servernum )
	...
	function OnOption3(){
		...
		if $result == 0
			...
			//????????????
			$num = $num + 1
			SetPlayerVar( -1 , 4058 , $num )
		
			//?????????????
			$servernum = GetServerVar( 274 )
			$servernum = $servernum + 1
			SetServerVar( 274 , $servernum )
