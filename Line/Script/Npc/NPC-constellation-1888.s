	...
	//		Modified:2009/07/15
	//		Author:???
	//		TaskName:NPC-constellation-1888.s
	//		TaskID:?? ????
	//
	//****************************************
	...
	function OnOption0(){
		...
		$a = GetServerVar( 133 )
		if $a == 0
			BC( "dialogbox", "player", -1, "\nToday's lucky constellation has not been decided yet.\n\nFind Arnyth to return to the constellation temple, and use Constellation Wish Charm to make the wish, in order to activate the constellation defender. The first constellation to gather 60 wishes will be the lucky constellation." )
			return
		endif
		if $a == 1
		...
		if $a == 3
			...
			$Status = IsExistStatus( -1 , 10685 )
			if $Status != 0
				BC( "dialogbox", "player", -1, "\nYou don't have the Gemini protection. You cannot acquire Gemini reward." )
				return
			endif
		...
		if $a == 6
			BC( "dialogbox", "player", -1, "\nToday's lucky constellation is Virgo!\n\nYou cannot acquire the lucky constellation reward. Use Constellation Faith Stamp to change your constellation protection to acquire the reward.")
			return
		endif
		if $a == 7
	...
	function OnOption1(){
		...
		if $Status3 == 0
			BC( "dialogbox", "player", -1, "You already have the Gemini protection. Do you want even more rewards?" )
			return
		endif
		if $Status2 == 0
			$a = GetPlayerInfo( -1 , "item", 63446 )
