...
//	?????1-12?????????
	...
	function OnOption0(){
		...
		$a = IsPlayerInTeam($id)
		if $a == -1
//			 BC( "chat", "player", -1, "GUID:03441000000" )
			 
			$result = BattleSign( 319 )
	...
	if $count < 5
		BC( "dialogbox", "player", -1, "You can get Glory for the first 5 Rounds of Ancient Ruins each day.\nYou've already participated in the Arena.", $count," time(s)\nAfter another", $temp_count," time(s), you will no longer receive Glory." )
	else
		BC( "dialogbox", "player", -1, "You've already participated in the Arena", $count," time(s).\nYou will not get any Glory if you continue, but you can still get Battlefield Score and EXP." )
	endif
