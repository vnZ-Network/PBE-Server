	...
	//		Modified:2010/03/15
	//		Author:???
	//		TaskName:god-task-item-64805.s
	//		TaskID:???64805
	//
	//****************************************
...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2543 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64805, 1 )
		if $result == 0
			AcceptTask ( -1, 2543 )
				//???????????
			if $use_time == $today_time
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Campaign Orders. You've received the Campaign Order-Berserker Spirit quest." )
		endif
