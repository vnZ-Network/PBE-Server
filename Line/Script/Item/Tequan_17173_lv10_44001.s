	...
	//		Author:???(??,??,??)
	//		TaskName:7??????17173?10???
	//		TaskID:Tequan-17173-lv10-44001.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44001, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36671, 1)
  else 
     AddPlayerInfo(-1, "item", 36670, 1)
  endif
