	...
	//------------????ID---------------
		if $mapid != 1
			BC( "screen", "player", -1, "According to the quest instruction, step 3 must be finished on Atlantic Ocean." )
			return
		endif
	//-------------------------------------------------------		
		if $player_x < 182
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 3 is not here." )
			return
		endif
		if $player_x > 192
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 3 is not here." )
	...
	//---------------???????????????-------------------------------	
		...
		//--------??item----------
		$info = SubPlayerInfo ( -1 , "item" , 12516 , 1 )
		if  $info != 0
			return
		endif
		$m = RandomNumber ( 0,1 )
