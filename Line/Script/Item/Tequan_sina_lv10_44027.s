	...
	//		Author:???(??,??,??)
	//		TaskName:7?????????10???
	//		TaskID:Tequan-sina-lv10-44027.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44027, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36683, 1)
  else 
     AddPlayerInfo(-1, "item", 36682, 1)
  endif
