	...
	//		Modified:2010/03/15
	//		Author:???
	//		TaskName:god-task-item-64806.s
	//		TaskID:???64806
	//
	//****************************************
...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2544 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64806, 1 )
		if $result == 0
			AcceptTask ( -1, 2544 )
				//???????????
			if $use_time == $today_time
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Campaign Orders. You've received the Campaign Order-Demigod Puppet quest." )
		endif
