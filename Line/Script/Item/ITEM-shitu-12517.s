	...
	//------------????ID---------------
		if $mapid != 7
			BC( "screen", "player", -1, "According to the quest instruction, step 4 must be finished in Bermuda Islands." )
			return
		endif
		//--------????????-----------------
		if $count < 2
	...
	//-------------------------------------------------------		
		if $player_x < 43
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 4 is not here." )
			return
		endif
		if $player_x > 53
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 4 is not here." )
	...
	//---------------???????????????-------------------------------	
		...
		//--------??item----------
		$info = SubPlayerInfo ( -1 , "item" , 12517 , 1 )
		if  $info != 0
			return
		endif
		$m = RandomNumber ( 0,1 )
