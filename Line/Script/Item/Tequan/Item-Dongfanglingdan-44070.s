	...
	//		Author:???(??,??,??)
	//		TaskName:??????
	//		TaskID:Item-Dongfanglingdan-44070.s
	//
	//****************************************
  ...
  $number = GetPlayerVar(-1, 3369)
  $Now_Yday = GetSystemTime("yday")
  $day = GetPlayerVar(-1, 3999)
  
  //????????,40???????????
  if $lv < 40
     BC("screen", "player", -1, "Let's try to reach LV40.")
     return
  endif
  ...
  if $day == $Now_Yday
	  if $number >= 6
	     BC("screen", "player", -1, "Bless you 6 times a day.")
	     return
	  endif
  endif
  ...
  //????
  $del_count = SubPlayerInfo(-1, "item", 44070, 1)
  if $del_count != 0
     return
  endif
  ...
  //??????
   if $day != $Now_Yday
     SetPlayerVar(-1, 3369, 1)
     SetPlayerVar(-1, 3999, $Now_Yday)
  else	  
     $number = $number + 1
     SetPlayerVar(-1, 3369, $number)
  endif
