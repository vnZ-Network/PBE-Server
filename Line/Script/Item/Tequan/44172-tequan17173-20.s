	...
	//		Modified:2012/03/15
	//		Author:???(??,??,??)
	//		TaskName:2012?4??????17173?20???
	//		TaskID:44172-tequan17173-20.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44172, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36972, 1)
  else 
     AddPlayerInfo(-1, "item", 36979, 1)
  endif
