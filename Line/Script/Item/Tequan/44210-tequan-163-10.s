...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44210, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 17)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36962, 1)
  else 
     AddPlayerInfo(-1, "item", 36969, 1)
  endif
