...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44204, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 16)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36961, 1)
  else 
     AddPlayerInfo(-1, "item", 36968, 1)
  endif
