...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44205, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36975, 1)
  else 
     AddPlayerInfo(-1, "item", 36982, 1)
  endif
