	...
	//		Modified:2012/03/15
	//		Author:???(??,??,??)
	//		TaskName:2012?4??????360?10???
	//		TaskID:44222-tequan-360-10.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44222, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 19)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36964, 1)
  else 
     AddPlayerInfo(-1, "item", 36971, 1)
  endif
