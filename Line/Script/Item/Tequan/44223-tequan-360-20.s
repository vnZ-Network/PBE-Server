	...
	//		Modified:2012/03/15
	//		Author:???(??,??,??)
	//		TaskName:2012?4??????360?20???
	//		TaskID:44223-tequan-360-20.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44223, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36978, 1)
  else 
     AddPlayerInfo(-1, "item", 36985, 1)
  endif
