	...
	//		Author:???(??,??,??)
	//		TaskName:2012?4????????????10???
	//		TaskID:44198-tequan-celebration-10.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44198, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 15)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36960, 1)
  else 
     AddPlayerInfo(-1, "item", 36967, 1)
  endif
