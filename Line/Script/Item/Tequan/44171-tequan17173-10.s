	...
	//		Modified:2012/03/15
	//		Author:???(??,??,??)
	//		TaskName:2012?4??????17173?10???
	//		TaskID:44171-tequan17173-10.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44171, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 11)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36958, 1)
  else 
     AddPlayerInfo(-1, "item", 36965, 1)
  endif
