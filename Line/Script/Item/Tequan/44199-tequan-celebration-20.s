	...
	//		Author:???(??,??,??)
	//		TaskName:2012?4?????????20???
	//		TaskID:44199-tequan-celebration-20.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44199, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36974, 1)
  else 
     AddPlayerInfo(-1, "item", 36981, 1)
  endif
