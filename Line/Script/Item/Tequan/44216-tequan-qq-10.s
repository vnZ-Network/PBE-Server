...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44216, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  SetPlayerVar(-1, 3371, 18)
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36963, 1)
  else 
     AddPlayerInfo(-1, "item", 36970, 1)
  endif
