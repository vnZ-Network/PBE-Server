	...
	//		Modified:2011/04/09
	//		Author:???
	//		TaskName:Tequan-Item-52PKlv30-59652.s
	//		TaskID:4????
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 59652, 1 )
  //??????????
  if $del_count != 0
     return
  endif
  ...
  //?????????
  AddPlayerInfo(-1, "item", 59653, 1 ) 
  //????,??????Buff
  PlayEffect(-1, "common\Yanhua\tx_Logo\tx_Logo_52PK.ini" )
  AddStatus(-1, 7583, 1 )
  //????????
  AddPlayerInfo(-1, "item", 21149, 1 )
