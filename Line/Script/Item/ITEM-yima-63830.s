	...
	function OnUseItem(){
		...
		//????
		$del_count = SubPlayerInfo( -1, "item", 63830, 1 )
		//??????????
		...
		if $del_count == 0
			...
			if $random_item_2 <= 2
				...
				BC("screen", "servergroup", -1, "The hero ",#name," has opened a Celestial Grace and received a Stardust Coin!" )
				BC("chat", "servergroup", -1, "The hero ",#name," has opened a Celestial Grace and received a Stardust Coin!" )
			endif
			
			if $random_item_2 > 2
				if $random_item_2 <= 3
					AddPlayerInfo( -1, "item", 62104, 1 )
					#name = GetPlayerInfo(-1 , "name")
					BC("screen", "servergroup", -1, "The hero ",#name," has opened a Celestial Grace and received a Pegasus Mount!" )
					BC("chat", "servergroup", -1, "The hero ",#name," has opened a Celestial Grace and received a Pegasus Mount!" )
				endif
			endif
