	...
	//		Modified:2009/11/26
	//		Author:???
	//		TaskName:rongyaoshop-Item-59093.s
	//		TaskID:?????????(???)
	//****************************************
	...
	function OnUseItem(){
		...
		$info = SubPlayerInfo( -1, "item", 59093 , 1  )
			if $info != 0
				return
			else
			  AddPlayerInfo( -1, "item", 60068 , 1  )
				BC( "screen" , "player" , -1 , "You got Bean KnightGolden Summon Card(Growth Card)" )
				BC( "chat" , "player" , -1 , "You got Bean KnightGolden Summon Card(Growth Card)" )
				return
			endif
