//****************************************
	...
	//		Author:???
	//		TaskName:???????
	//		TaskID:44511-tequan-Commander-10.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  if $level < 70
     BC( "screen", "player", -1, "You have to be LV70 before you can open the Glory Guild Deluxe Pack!" )
     BC( "chat", "player", -1, "You have to be LV70 before you can open the Glory Guild Deluxe Pack!" )
     return
  endif
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44511, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  AddPlayerInfo(-1,"item",59557,8)
	...
	AddPlayerInfo(-1,"item",50036,200)
