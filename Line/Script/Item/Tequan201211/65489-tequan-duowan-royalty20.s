	...
	//		Author:???(??,??,??)
	//		TaskName:2012?8???????????20???
	//		TaskID:44295-tequan-duowan-royalty20.s
	//
	//****************************************
...
//  $del_count = SubPlayerInfo( -1, "item", 44295, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  AddPlayerInfo(-1, "item", 44296, 1 ) 
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36139, 1)
  else 
     AddPlayerInfo(-1, "item", 36138, 1)
  endif
