	...
	//		Author:???(??,??,??)
	//		TaskName:2012?8?????????20???
	//		TaskID:44315-tequan-celebration-20.s
	//
	//****************************************
...
//  $del_count = SubPlayerInfo( -1, "item", 44315, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  AddPlayerInfo(-1, "item", 44316, 1 ) 
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36151, 1)
  else 
     AddPlayerInfo(-1, "item", 36150, 1)
  endif
