	...
	//		Author:???(??,??,??)
	//		TaskName:2012?8???????????20???
	//		TaskID:44302-tequan-duowan-noble20.s
	//
	//****************************************
...
//  $del_count = SubPlayerInfo( -1, "item", 44302, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
...
//  AddPlayerInfo(-1, "item", 44303, 1 ) 
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36143, 1)
  else 
     AddPlayerInfo(-1, "item", 36142, 1)
  endif
