//****************************************
	...
	//		Author:???
	//		TaskName:???????
	//		TaskID:44507-tequan-Commander-10.s
	//
	//****************************************
...
function OnUseItem(){
 ...
 //
 //if $is_used == 1
 //   BC("screen", "player", -1, "GUID:07527000000")
 //   BC("chat", "player", -1, "GUID:07527000001")
 //   return
 //endif
  ...
  if $level < 50
     BC( "screen", "player", -1, "You have to be LV50 before you can open the Glory Guild Pack!" )
     BC( "chat", "player", -1, "You have to be LV50 before you can open the Glory Guild Pack!" )
     return
  endif
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44507, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
