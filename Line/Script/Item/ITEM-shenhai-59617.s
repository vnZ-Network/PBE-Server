	...
	function OnUseItem(){
		...
		//????
		$del_count = SubPlayerInfo( -1, "item", 59617, 1 )
		//??????????
		...
		if $del_count == 0
			...
			if $random_item_2 <= 2
				...
				BC("screen", "servergroup", -1, "The hero ",#name," has opened an Abysmal Shadow and received a Stardust Coin!" )
				BC("chat", "servergroup", -1, "The hero ",#name," has opened an Abysmal Shadow and received a Stardust Coin!" )
			endif
			
			if $random_item_2 > 2
				if $random_item_2 <= 3
					AddPlayerInfo( -1, "item", 62110, 1 )
					#name = GetPlayerInfo(-1 , "name")
					BC("screen", "servergroup", -1, "The hero ",#name," has opened an Abysmal Shadow and received a Stoneskin Shark Mount!" )
					BC("chat", "servergroup", -1, "The hero ",#name," has opened an Abysmal Shadow and received a Stoneskin Shark Mount!" )
				endif
			endif
