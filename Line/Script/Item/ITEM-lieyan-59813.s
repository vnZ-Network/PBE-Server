	...
	function OnUseItem(){
		...
		//????
		$del_count = SubPlayerInfo( -1, "item", 59813, 1 )
		//??????????
		...
		if $del_count == 0
			...
			if $random_item_2 <= 2
				...
				BC("screen", "servergroup", -1, "The hero ",#name," has used Flame Roar and have acquired Stardust Coins" )
				BC("chat", "servergroup", -1, "The hero ",#name," has used Flame Roar and have acquired Stardust Coins" )
			endif
			
			if $random_item_2 > 2
				if $random_item_2 <= 3
					AddPlayerInfo( -1, "item", 62113, 1 )
					#name = GetPlayerInfo(-1 , "name")
					BC("screen", "servergroup", -1, "The hero ",#name," has used Flame Roar and have acquired a Demonlord mount." )
					BC("chat", "servergroup", -1, "The hero ",#name," has used Flame Roar and have acquired a Demonlord mount." )
				endif
			endif
