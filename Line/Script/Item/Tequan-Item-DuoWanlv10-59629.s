	...
	//		Modified:2011/04/09
	//		Author:???
	//		TaskName:Tequan-Item-DuoWanlv10-59629.s
	//		TaskID:4????
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 59629, 1 )
  //??????????
  if $del_count != 0
     return
  endif
  ...
  //???????????
  SetPlayerVar(-1, 3385, 1)
  //?????????
  AddPlayerInfo(-1, "item", 59630, 1 ) 
  //????,??????Buff
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36655, 1)
  else 
     AddPlayerInfo(-1, "item", 36654, 1)
  endif
