...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44326, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  //???2012?17173,??11
  SetPlayerVar(-1, 3381, 17)

  //---------?????????------------
  
  //?????????
  AddPlayerInfo(-1, "item", 44327, 1 ) 
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36117, 1)
  else 
     AddPlayerInfo(-1, "item", 36116, 1)
  endif
