	...
	//		Author:???(??,??,??)
	//		TaskName:7?????????10???
	//		TaskID:Tequan-duowan-lv10-44008.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44008, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36675, 1)
  else 
     AddPlayerInfo(-1, "item", 36674, 1)
  endif
