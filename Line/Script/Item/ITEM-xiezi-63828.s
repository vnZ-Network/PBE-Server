	...
	function OnUseItem(){
		...
		//????
		$del_count = SubPlayerInfo( -1, "item", 63828, 1 )
		//??????????
		...
		if $del_count == 0
			...
			if $random_item_2 <= 2
				...
				BC("screen", "servergroup", -1, "The hero ",#name,"You used a Violent Tremor and got a Stardust Coin!" )
				BC("chat", "servergroup", -1, "The hero ",#name,"You used a Violent Tremor and got a Stardust Coin!" )
			endif
			
			if $random_item_2 > 2
				if $random_item_2 <= 3
					AddPlayerInfo( -1, "item", 62079, 1 )
					#name = GetPlayerInfo(-1 , "name")
					BC("screen", "servergroup", -1, "The hero ",#name,"You used a Violent Tremor and got a Thunder Scorpion Mount!" )
					BC("chat", "servergroup", -1, "The hero ",#name,"You used a Violent Tremor and got a Thunder Scorpion Mount!" )
				endif
			endif
