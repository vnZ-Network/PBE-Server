	...
	//		Modified:2009/11/26
	//		Author:???
	//		TaskName:rongyaoshop-Item-59088.s
	//		TaskID:??????????
	//****************************************
	...
	function OnUseItem(){
		...
		$info = SubPlayerInfo( -1, "item", 59088 , 1  )
			if $info != 0
				return
			else
			  AddPlayerInfo( -1, "item", 61117 , 1  )
				BC( "screen" , "player" , -1 , "You got [Action] exultation" )
				BC( "chat" , "player" , -1 , "You got [Action] exultation" )
				return
			endif
