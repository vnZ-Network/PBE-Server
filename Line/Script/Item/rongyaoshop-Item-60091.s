	...
	//		Modified:2009/11/26
	//		Author:???
	//		TaskName:rongyaoshop-Item-60091.s
	//		TaskID:??????(???)
	//****************************************
	...
	function OnUseItem(){
		...
		if $title1 != 0
					BC( "screen" , "player" , -1 , "You do not have the title of Master level 4, upgrade your master level with Gallaja in Atlantis!" )
					BC( "chat" , "player" , -1 , "You do not have the title of Master level 4, upgrade your master level with Gallaja in Atlantis!" )
					return
		endif
		...
		if $time != $date
			$info = SubPlayerInfo( -1, "item", 60091 , 1  )
			if $info != 0
				return
			endif
