	...
	function OnUseItem(){
		...
		//????
		$del_count = SubPlayerInfo( -1, "item", 63754, 1 )
		//??????????
		...
		if $del_count == 0
			...
			if $random_item_2 <= 2
				...
				BC("screen", "servergroup", -1, "The hero ",#name," has activated an Aurora Shield and acquired a Stardust Coin!" )
				BC("chat", "servergroup", -1, "The hero ",#name," has activated an Aurora Shield and acquired a Stardust Coin!" )
			endif
			...
			if $random_item_2 > 2
				if $random_item_2 <= 3
					...
					#name = GetPlayerInfo(-1 , "name")
					BC("screen", "servergroup", -1, "The hero ",#name," has activated an Aurora Shield and acquired Man of Iron!" )
					BC("chat", "servergroup", -1, "The hero ",#name," has activated an Aurora Shield and acquired Man of Iron!" )
				endif
			endif
