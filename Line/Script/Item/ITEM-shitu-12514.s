	...
	//--------????????-----------------
		if $count < 2
			BC( "dialogbox", "player", -1, "To start step 1, the master must be the party leader, and both the master and the disciples must be in the party without any other players." )
			return
		endif	
		
	//------------????ID---------------
		if $mapid != 86
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 1 is not here." )
			return
		endif
	//-------------------------------------------------------		
		if $player_x < 230
	...
	//---------------???????????????-------------------------------	
		...
		//--------??item----------
		$info = SubPlayerInfo ( -1 , "item" , 12514 , 1 )
		if  $info != 0
			return
		endif
		$m = RandomNumber ( 0,1 )
			...
			if $level > 44
				if $level < 55
					if $n < 3
						...
						BC( "screen", "player", -1, "A scary Piggish Catty has just showed up!" )
					endif
				endif
			endif
			if $level > 54
