	...
	//------------????ID---------------
		if $mapid != 6
			BC( "screen", "player", -1, "According to the quest instruction, step 6 must be finished in Grand Rift." )
			return
		endif
	//-------------------------------------------------------		
		if $player_x < 266
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 6 is not here." )
			return
		endif
		if $player_x > 276
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 6 is not here." )
	...
	//---------------???????????????-------------------------------	
		...
		//--------??item----------
		$info = SubPlayerInfo ( -1 , "item" , 12519 , 1 )
		if  $info != 0
			return
		endif
		$m = RandomNumber ( 0,1 )
