	...
	//------------????ID---------------
		if $mapid != 106
			BC( "screen", "player", -1, "According to the quest instruction, step 2 must be done in Easter Island South." )
			return
		endif
	//-------------------------------------------------------		
		if $player_x < 60
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 2 is not here." )
			return
		endif
		if $player_x > 70
			BC( "screen", "player", -1, "According to the quest instruction, the requisite location of step 2 is not here." )
	...
	//---------------???????????????-------------------------------	
		...
		//--------??item----------
		$info = SubPlayerInfo ( -1 , "item" , 12515 , 1 )
		if  $info != 0
			return
		endif
		$m = RandomNumber ( 0,1 )
