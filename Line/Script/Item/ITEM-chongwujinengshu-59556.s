	...
	//		Modified:2009/5/20
	//		Author:??
	//		TaskName:ITEM-chongwujinengshu-59556.s
	//		TaskID:???????
	//
	//****************************************
	...
	function OnUseItem(){
		...
		$count1 = GetPlayerInfo( -1, "item", 59556 )
		$count2 = GetPlayerInfo( -1, "item", 63381 )
		$count = $count1 + $count2
		if $count < 20
		...
		if $count1 >= 20	
			$del_count1 = SubPlayerInfo( -1, "item", 59556, 20 )
		endif
		
		if $count1 < 20
			$del_countno = 20 - $count1
			$del_count1s = SubPlayerInfo( -1, "item", 59556, $count1 )
			$del_count2s = SubPlayerInfo( -1, "item", 63381, $del_countno )
		endif					
