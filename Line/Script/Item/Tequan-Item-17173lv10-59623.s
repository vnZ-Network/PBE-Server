	...
	//		Modified:2011/04/09
	//		Author:???
	//		TaskName:Tequan-Item-17173lv10-59623.s
	//		TaskID:4????
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 59623, 1 )
  //??????????
  if $del_count != 0
     return
  endif
  ...
  //???????????
  SetPlayerVar(-1, 3384, 1)
  //?????????
  AddPlayerInfo(-1, "item", 59624, 1 ) 
  //????,??????Buff
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36651, 1)
  else 
     AddPlayerInfo(-1, "item", 36650, 1)
  endif
