	...
	//		Modified:2010/03/15
	//		Author:???
	//		TaskName:Skill-task-item-59276.s
	//		TaskID:????-??
	//
	//****************************************
...
function OnUseItem(){
		...
		if $profession != 4
			BC( "screen", "player", -1, "Sorry! Only Magi can learn the Magic Aura skill." )
			return
		endif
		
//		$godlevel = GetGodLevel( -1 )
//		if $godlevel < 5
//			BC( "screen", "player", -1, "GUID:07310000001" )
//			return
//		endif
		...
		$skill = GetSkillLevel( -1, 8322 )
		if $skill > 0
			BC( "screen", "player", -1, "You have already learned this skill!" )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 59276, 1 )
		if $result != 0
			return
		endif
