...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2478)
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return	
		endif
		
		$result = SubPlayerInfo( -1, "item", 64678, 1 )
		if $result == 0
			AcceptTask ( -1, 2478 )
				//???????????
			if $use_time == $today_time
...
SetPlayerVar( -1, 3624, $number )
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Crusade Orders. You have acquired the quest Crusade Order - Ashura Berserker" )
		endif
	}	