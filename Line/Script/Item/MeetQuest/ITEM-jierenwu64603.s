...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2403 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64603, 1 )
		if $result == 0
			AcceptTask ( -1, 2403 )
				//???????????
			if $use_time == $today_time
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Crusade Orders. You have acquired the quest Crusade Order - Angry Pompeiian" )
		endif
	}