...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2452 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64652, 1 )
		if $result == 0
			AcceptTask ( -1, 2452 )
				//???????????
			if $use_time == $today_time
...
SetPlayerVar( -1, 3624, $number )
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Crusade Orders. You have acquired the quest Crusade Order - Lizard Grinder" )
		endif
	}