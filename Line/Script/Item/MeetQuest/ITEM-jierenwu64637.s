...
function OnUseItem(){
		...
		$result = SubPlayerInfo( -1, "item", 64637, 1 )
		if $result == 0
			AcceptTask ( -1, 2437 )
				//???????????
			if $use_time == $today_time
...
SetPlayerVar( -1, 3624, $number )
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Crusade Orders. You have acquired the quest Crusade Order - Pharaoh Squire" )
		endif
	}