...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2436 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64636, 1 )
		if $result == 0
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Crusade Orders. You have acquired the quest Crusade Order - Gold Beetle" )
		endif
	}