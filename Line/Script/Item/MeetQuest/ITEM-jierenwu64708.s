...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2520 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64708, 1 )
		if $result == 0
			AcceptTask ( -1, 2520 )
				//???????????
			if $use_time == $today_time
			...
			BC( "messagebox", "player", -1, "You have used ",$number,"You took the quest Quest-Crusade-Subterranean Soul." )
		endif
	}