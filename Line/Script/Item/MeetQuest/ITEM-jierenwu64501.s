	...
	function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 832 )
		if $result == 0
			BC( "screen", "player", -1, "You have that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64501, 1 )
		if $result == 0
			AcceptTask ( -1, 832 )
			BC( "messagebox", "player", -1, "You have acquired the Sealed Letter quest." )
		endif
