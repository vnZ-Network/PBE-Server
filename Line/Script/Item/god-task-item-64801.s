	...
	//		Modified:2010/03/15
	//		Author:???
	//		TaskName:god-task-item-64801.s
	//		TaskID:???64801
	//
	//****************************************
...
function OnUseItem(){
		...
		$result = IsTaskAccept( -1, 2539 )
		if $result == 0
			BC( "screen", "player", -1, "You have acquired that quest already." )
			return
		endif
		
		$result = SubPlayerInfo( -1, "item", 64801, 1 )
		if $result == 0
			AcceptTask ( -1, 2539 )
				//???????????
			if $use_time == $today_time
			...
			BC( "messagebox", "player", -1, "You have used ",$number," Campaign Orders. You've received the Campaign Order-Harpy quest." )
		endif
