	...
	//		Author:???(??,??,??)
	//		TaskName:7?????????10???
	//		TaskID:Tequan-phone-lv10-44039.s
	//
	//****************************************
...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44039, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36691, 1)
  else 
     AddPlayerInfo(-1, "item", 36690, 1)
  endif
