...
function OnUseItem(){
  ...
  //???????
  $del_count = SubPlayerInfo( -1, "item", 44021, 1 )
  //??????????,????????
  if $del_count != 0
     return
  endif
  ...
  if $gender == 0
     AddPlayerInfo(-1, "item", 36679, 1)
  else 
     AddPlayerInfo(-1, "item", 36678, 1)
  endif
