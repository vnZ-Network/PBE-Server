	...
	//		Modified:2010/03/17
	//		Author:???
	//		TaskName:Ectype-elinggubao-Npc-2147.s
	//		TaskID:???4
	//
	//****************************************
	...
	function OnOption0(){
		...
		if $d == 0
			...
			BC( "screen" , "map" , $ectype_id , "The statue shakes fiercely! Black Castle Guards will appear in 10 seconds!" )
		else
			if $d == 1
				BC( "screen" , "player" , -1 , "Kill all the Black Castle Guards to activate Statue IV." )
				return
			endif
			
			if $d == 10
				BC( "screen" , "player" , -1  , "Statue IV activated!" )
				return	
			endif
		endif
