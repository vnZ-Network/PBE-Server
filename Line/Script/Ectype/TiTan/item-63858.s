	...
	function OnUseItem(){
		...
		if $bag_count < 1
	 	...
	 	$result = SubPlayerInfo( -1, "item", 63858, 1)
		if $result == 0
			$player_profession = GetPlayerInfo(-1,"profession")
//1:??-//2:??-//3:??-//4:??-//5:??-//6:???
			if $player_profession == 1
				AddPlayerInfo( -1, "item", 19621, 1)
			endif
			if $player_profession == 2
				AddPlayerInfo( -1, "item", 19622, 1)
			endif
			if $player_profession == 3
				AddPlayerInfo( -1, "item", 19623, 1)
			endif
			if $player_profession == 4
				AddPlayerInfo( -1, "item", 19624, 1)
			endif
			if $player_profession == 5
				AddPlayerInfo( -1, "item", 19625, 1)
			endif
			if $player_profession == 6
				AddPlayerInfo( -1, "item", 19585, 1)
			endif
//----------------------------------------------------------??
		endif
