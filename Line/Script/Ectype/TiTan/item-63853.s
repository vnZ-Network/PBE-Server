	...
	function OnUseItem(){
		...
		if $bag_count < 1
	 	...
	 	$result = SubPlayerInfo( -1, "item", 63853, 1)
		if $result == 0
			$player_profession = GetPlayerInfo(-1,"profession")
//1:??-//2:??-//3:??-//4:??-//5:??-//6:???
			if $player_profession == 1
				AddPlayerInfo( -1, "item", 28206, 1)
			endif
			if $player_profession == 2
				AddPlayerInfo( -1, "item", 28216, 1)
			endif
			if $player_profession == 3
				AddPlayerInfo( -1, "item", 28226, 1)
			endif
			if $player_profession == 4
				AddPlayerInfo( -1, "item", 28236, 1)
			endif
			if $player_profession == 5
				AddPlayerInfo( -1, "item", 28246, 1)
			endif
			if $player_profession == 6
				AddPlayerInfo( -1, "item", 31245, 1)
			endif
//----------------------------------------------------------?
		endif
