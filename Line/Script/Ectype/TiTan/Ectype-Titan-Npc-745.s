	...
	//		Modified:2010/08/11
	//		Author:???
	//		TaskName:Ectype-Titan-Npc-745.s
	//		TaskID:????????
	//
	//****************************************
	//
	...
	function OnOption0(){
		...
		$is_succeed = FlyToEctype( $team_headerID, 332, 88, 108 )
		if $is_succeed > 0
			SetPlayerVar( $team_headerID, 3833, $is_succeed )
			BC( "screen", "player", $team_headerID, "You have entered the Upper Titan Throne. " )
			//????ID
			$ectype_IDM = GetEctypeID( $team_headerID, 313 )
			$ectype_IDL = GetEctypeID( $team_headerID, 332 )
			$ectype_type = GetEctypeVar( $ectype_IDM, 0 )
			StartEctypeTimer( $team_headerID, 332, 0 )
			StartEctypeTimer( $team_headerID, 313, 1 )
			//??????
			SetEctypeVar( $ectype_IDL, 0, $ectype_type )
		else
		...
		if $teamplayer1_id != -1
			if $teamplayer1_id != $team_headerID		
				$is_succeed = FlyToEctype( $teamplayer1_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer1_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer1_id, "You have entered the Upper Titan Throne. " )
				else
		...
		if $teamplayer2_id != -1
			if $teamplayer2_id != $team_headerID		
				$is_succeed = FlyToEctype( $teamplayer2_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer2_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer2_id, "You have entered the Upper Titan Throne. " )
				else
		...
		if $teamplayer3_id != -1
			if $teamplayer3_id != $team_headerID			
				$is_succeed = FlyToEctype( $teamplayer3_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer3_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer3_id, "You have entered the Upper Titan Throne. " )
				else
		...
		if $teamplayer4_id != -1
			if $teamplayer4_id != $team_headerID		
				$is_succeed = FlyToEctype( $teamplayer4_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer4_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer4_id, "You have entered the Upper Titan Throne. " )
				else
		...
		if $teamplayer5_id != -1
			if $teamplayer5_id != $team_headerID		
				$is_succeed = FlyToEctype( $teamplayer5_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer5_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer5_id, "You have entered the Upper Titan Throne. " )
				else
		...
		if $teamplayer6_id != -1
			if $teamplayer6_id != $team_headerID		
				$is_succeed = FlyToEctype( $teamplayer6_id, 332, 88, 108 )
				if $is_succeed > 0
					SetPlayerVar( $teamplayer6_id , 3833 , $is_succeed )
					BC( "screen", "player", $teamplayer6_id, "You have entered the Upper Titan Throne. " )
				else
