	...
	//		Modified:2009/11/25
	//		Author: ÌÆÒú
	//		TaskName:Ectype-nengliangzhijing-Monster-51356.s
	//		TaskID:ËÀÍö
	//****************************************
	...
	//----------µôÂä°ü
			...
			if $teamplayer1_id != -1
				...
				if $Map1 == $ectype_ID			
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer1_id, "prestige", 44, 160)
			...
			if $teamplayer2_id != -1
				...
				if $Map2 == $ectype_ID				
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer2_id, "prestige", 44, 160)
			...
			if $teamplayer3_id != -1
				...
				if $Map3 == $ectype_ID			
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer3_id, "prestige", 44, 160)
			...
			if $teamplayer4_id != -1
				...
				if $Map4 == $ectype_ID				
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer4_id, "prestige", 44, 160)
			...
			if $teamplayer5_id != -1
				...
				if $Map5 == $ectype_ID				
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer5_id, "prestige", 44, 160)
			...
			if $teamplayer6_id != -1
				...
				if $Map6 == $ectype_ID			
					if $len <= 60
						if $team_level <= 90
							AddPlayerInfo($teamplayer6_id, "prestige", 44, 160)
