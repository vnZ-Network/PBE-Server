	...
	//?? ????51800 ?????? 51781 51782 ???? 51776 NPC??596
	//???? 51783 ???? 51842
	$ectype_id = GetEctypeID(-1, 479)
	$sum = GetEctypeVar(-1, 3)
	$line = GetEctypeVar(-1, 13)
	...
	//?????????
		if $sum != 2
			BC( "screen", "player", -1, "No response from the 5th Shrine of Oath. You pledged to the wrong shrine.")
			BC( "screen", "map", $ectype_id, "You pledged to the wrong shrine and offended the Ancestral Spirit!")
	//???????
			AddMonsterByFloat(51783, 1, $ectype_id, 299, 199, 0)
			return
		else
	//??NPC??
	...
	//????????
			...
			$check = GetEctypeVar(-1, 8)
//			BC( "screen", "map", $ectype_id, "GUID:04067000008" , $check , " ")	
		endif
	endif		
	...
	//?????????
		if $sum != 8
			...
			BC( "screen", "map", $ectype_id, "You pledged to the wrong shrine and offended the Ancestral Spirit!")
	//???????
			AddMonsterByFloat(51783, 1, $ectype_id, 299, 195, 0)
			return
		else
	//??NPC??
			DeleteNPC(602, $ectype_id)
			...
			BC("messagebox","player",-1,"You have pledged your oath to the imperial army. Accept the Oath Sword as your reward.")
	//??12???
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 8)
//			BC( "screen", "map", $ectype_id, "GUID:04067000015" , $check , " ")	
	//???????,??????-????,????,????,??????
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 8)
//			BC( "screen", "map", $ectype_id, "GUID:04067000022" , $check , " ")	
	//???????,??????-????,????,????,??????
			OpenMask(20, $ectype_id)
			AddMonsterByFloat( 51810, 1, $ectype_id, 290, 156, 0, 0, 180)
			AddMonsterByFloat( 51810, 1, $ectype_id, 143, 170, 0, 0, 270)
