	...
	//		Author:???(??,??,??)
	//		TaskName:???479??3??NPC????
	//		TaskID:DXLM3-JGNPC-ZhenyanGreen-619.s
	//
	//****************************************

	function OnRequest(){

	//???? ??id 479,?????? 14--0?1?,????? 19,??13--123? 1?RB10YG14,2?RB11YG15,3??12
	//??????????,?????,???Option0
	...
	if $line == 1
  	if $gree != 14
  		...
  		BC( "screen", "map", $ectype_id, "The Green Seal's energy is just right. If you activate the Red-Green-Blue Seal, the balance of energy will be broken.")
  	endif
  endif
  ...
  if $line == 3
	...
	//???? ??id 479,??13--123?,??4????? ?16 ?17 ?18 ?19----????? ?+1?+0?+1?+1
	//??id ???? 51781 ???? 51782
	$ectype_id = GetEctypeID(-1, 479)
	$line = GetEctypeVar(-1, 13)
	$red = GetEctypeVar(-1, 16)
  ...
  //----------------------------  ? 10  ----------------------------
	...
	//??????14,???????
		if $gree == 14
			BC( "screen", "map", $ectype_id, "The Green Seal has been opened. If another colored Seal was opened, you can go to the black Seal to remove the seal of Three Ancestors' soul.")
			return
		endif
	endif
  ...
  //----------------------------  ? 12  ----------------------------
	...
	if $revaluet != 0
		...
		BC( "screen", "map", $ectype_id, "The Green Seal has been activated.")
	...
	//----------------------------  ? +1  ----------------------------
		...
		BC( "screen", "map", $ectype_id, "Red, Blue, and Green Seal- adds 2 energy. Yellow Seal- adds 0 energy.")

		AddMonster($mst_1, 1, $ectype_id, 101, 80, 4)
		AddMonster($mst_2, 1, $ectype_id, 101, 80, 4)
	endif
