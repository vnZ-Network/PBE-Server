	...
	if $is_44058 == 10
		BC( "screen", "player", -1, "It seems someone has already taken the Oath Sword.")
		return
	endif
	...
	BC( "screen", "map", $ectype_id, "Pledge your oath to receive the Oath Sword.")
	...
	if $is_call == 1
		BC( "dialogbox", "player", -1, "I summoned the monsters who swallowed the three Ancestor's soul. Please help me kill them.")
		return
	endif 
	...
	//????????,????????	
	if $is_done != 1
		BC( "screen", "map", $ectype_id, "Go upstairs and defeat Raendys!")
		return
	endif

	//?????????,???????????
	if $is_done1 != 1
		BC( "screen", "map", $ectype_id, "Because there are still monsters alive, you can not get the Tiger Talisman.")	
		return
	endif
	...
	//??????
	if $null < 1
		BC( "screen", "player", -1, "Your bag is full. You have no space for the Tiger Talisman")
		return
	endif
	...
	if $revaluet == 0
		BC( "screen", "player", -1, "The Tiger Talisman can command thousands of soldiers. You will rule the world!")
