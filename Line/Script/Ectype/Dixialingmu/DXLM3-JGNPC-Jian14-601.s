	...
	//?????????
		if $sum != 3
			BC( "screen", "player", -1, "No response from the 4th Shrine of Oath. You pledged to the wrong shrine.")
			BC( "screen", "map", $ectype_id, "You pledged to the wrong shrine and offended the Ancestral Spirit!")
	//???????
			AddMonsterByFloat(51783, 1, $ectype_id, 289, 195, 0)
			return
		else
	//??NPC??
	...
	//????????
			...
			$check = GetEctypeVar(-1, 7)
//		BC( "screen", "map", $ectype_id, "GUID:04066000010" , $check , " ")	
		endif
	endif		
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 7)
//			BC( "screen", "map", $ectype_id, "GUID:04066000016" , $check , " ")	
		endif
	endif
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 7)
//			BC( "screen", "map", $ectype_id, "GUID:04066000022" , $check , " ")	
		endif
	endif	
