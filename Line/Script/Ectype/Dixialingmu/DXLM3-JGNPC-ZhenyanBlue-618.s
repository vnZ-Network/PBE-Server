	...
	//		Author:???(??,??,??)
	//		TaskName:???479??3??NPC????
	//		TaskID:DXLM3-JGNPC-ZhenyanBlue-618.s
	//
	//****************************************

	function OnRequest(){

	//???? ??id 479,?????? 14--0?1?,????? 18,??13--123? 1?RB10YG14,2?RB11YG15,3??12
	//??????????,?????,???Option0
	...
	if $line == 1
  	if $blue != 10
  		...
  		BC( "screen", "player", -1, "Current energy point Red Seal--" ,$red, " Yellow Seal--" ,$yell, " Blue Seal--" ,$blue, " Green Seal--" ,$gree, "")
  	else
  		BC( "screen", "map", $ectype_id, "The Blue Seal's energy is just right. If you activate the Yellow-Blue-Green Seal, the balance of energy will be broken.")
  	endif
  endif
  
	if $line == 2
  	if $blue != 12
  		BC( "screen", "player", -1, "Blue Seal's energy is" , $blue, "point")
  ...
  if $line == 3
  	if $blue != 12
  		BC( "screen", "player", -1, "Blue Seal's energy is" , $blue, "point")
  		BC( "screen", "player", -1, "Energy balance Red array--12 Yellow array--12 Blue array--12 Green array--12")
  		BC( "screen", "player", -1, "Current energy point Red Seal--" ,$red, " Yellow Seal--" ,$yell, " Blue Seal--" ,$blue, " Green Seal--" ,$gree, "")
  	else
  		BC( "screen", "map", $ectype_id, "The Blue Seal's energy is just right. If you activate the Yellow-Blue-Green Seal, the balance of energy will be broken.")
  	endif
	...
	//???? ??id 479,??13--123?,??4????? ?16 ?17 ?18 ?19----????? ?+0?+2?+2?+2
	//??id ???? 51781 ???? 51782
	$ectype_id = GetEctypeID(-1, 479)
	$line = GetEctypeVar(-1, 13)
	$red = GetEctypeVar(-1, 16)
  ...
  //----------------------------  ? 10  ----------------------------
	...
	//??????10,???????
		if $blue == 10
			BC( "screen", "map", $ectype_id, "The Blue Seal has been opened. If another colored Seal was opened, you can go to the black Seal to remove the seal of Three Ancestors' soul.")
			return
		endif
	//----------------------------  ? 14  ----------------------------
		if $gree > 14
  ...
  //----------------------------  ? 12  ----------------------------
	...
	if $revaluet != 0
		...
		BC( "screen", "player", -1, "The Blue Seal has been activated successfully.")
	...
	//----------------------------  ? +2  ----------------------------
		...
		BC( "screen", "map", $ectype_id, "Yellow, Blue, and Green Seal- adds 2 energy. Red Seal- adds 0 energy.")
