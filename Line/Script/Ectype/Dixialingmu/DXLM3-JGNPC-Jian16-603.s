	...
	//?????????
		if $sum != 6
			BC( "screen", "player", -1, "No response from the 6th Shrine of Oath. You pledged to the wrong shrine.")
			BC( "screen", "map", $ectype_id, "You pledged to the wrong shrine and offended the Ancestral Spirit!")
	//???????
			AddMonsterByFloat(51783, 1, $ectype_id, 309, 195, 0)
			return
		else
	//??NPC??
	...
	//????????
			...
			$check = GetEctypeVar(-1, 9)
//			BC( "screen", "map", $ectype_id, "GUID:04068000010" , $check , " ")	
		endif
	endif		
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 9)
//			BC( "screen", "map", $ectype_id, "GUID:04068000016" , $check , " ")	
		endif
	endif
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")
			$check = GetEctypeVar(-1, 9)
//			BC( "screen", "map", $ectype_id, "GUID:04068000022" , $check , " ")	
		endif
	endif	
