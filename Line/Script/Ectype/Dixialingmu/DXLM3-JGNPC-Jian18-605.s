	...
	//???? ??id 479,??3--0~10 ??????11--0?1?,??13--1?2?3?
	$ectype_id = GetEctypeID(-1, 479)
	$sum = GetEctypeVar(-1, 3)
	$is = GetEctypeVar(-1, 11)
	...
	//?????????
		if $sum != 7
			BC( "screen", "player", -1, "No response from the 8th Shrine of Oath. You pledged to the wrong shrine.")
			BC( "screen", "map", $ectype_id, "You pledged to the wrong shrine and offended the Ancestral Spirit!")
	//???????
			AddMonsterByFloat(51783, 1, $ectype_id, 299, 208, 0)
			return
		else
	//??NPC??
			DeleteNPC(605, $ectype_id)
			AddNPC(614, $ectype_id)
			BC( "screen", "map", $ectype_id, "You have successfully pledged to the 8th Shrine of Oath.")
	...
	//????????
			...
			$check = GetEctypeVar(-1, 11)
//			BC( "screen", "map", $ectype_id, "GUID:04070000010" , $check , " ")	
		endif
	endif		
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")	
			$check = GetEctypeVar(-1, 11)
//			BC( "screen", "map", $ectype_id, "GUID:04070000016" , $check , " ")	
		endif
	endif
	...
	//????????
			...
			BC( "screen", "map", $ectype_id, "Number of shrines pledged to: " , $sum , " ")	
			$check = GetEctypeVar(-1, 11)
//			BC( "screen", "map", $ectype_id, "GUID:04070000022" , $check , " ")	
		endif
	endif	
